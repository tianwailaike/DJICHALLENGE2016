...
#include <../../build/devel/include/dji_sdk/DetectionPoints.h>
#include <ros/ros.h>
...
private:
  ...
  ros::ServiceClient send_data_to_remote_device_service; //Add a client for send_data_to_remote_device_service (not binding yet)
...
//Add a subscriber to obtain the detected Tag's position in an image;
  ros::Subscriber m_detectionPoints_subscriber;
//#ifdef PID_USED
  //Add subscriber to get control results from pid, y, z directions
  ros::Subscriber y_control_effort_sub, z_control_effort_sub;
//#else
...
public:
  ...
  //Received data msg from mobile
  dji_sdk::TransparentTransmissionData transparentdata;
  //Received the position of the detected Tag in an image;

  dji_sdk::DetectionPoints detection_points;
  //For controlling flight using detection results delivered from apriltag_detection_pose
  float flight_x, flight_y, flight_x_tag,flight_y_tag,flight_norm,flight_height, flight_yaw,flight_gimbal_angle_inc;
...
//Add PID control effort callbacks
  void control_y_fromPID_callback ( const std_msgs::Float64 & control_effort_y )
  {
    // ROS_INFO ( "y:= %0.3f",control_effort_y.data );
    this->flight_y = - control_effort_y.data;
  }
  void control_z_fromPID_callback ( const std_msgs::Float64 & control_effort_z )
  {
    this->flight_x =  control_effort_z.data;
...
//Add a callback for apriltag_detetion
  void apriltag_datection_callback ( const dji_sdk::Reldist & detection ) // geometry_msgs::PoseWithCovarianceStamped &msg)
  {
   ...
   this->flight_y_tag = -detection.y;
    this->flight_gimbal_angle_inc = detection.gimbal_inc;// = atan(detection.z/(detection.x+0.00000001))*57.3;//To angle
  ...
  //
  void received_detectionPoints_callback ( const dji_sdk::DetectionPoints & detectionPoints )//use const!
  {
...
public:
  DJIDrone ( ros::NodeHandle& nh ) :
    ...
    flight_x ( 0.0 ), //Initial flight data
    flight_y ( 0.0 ), //
    flight_norm ( 6.0 ), //
    flight_height(6.0)
  {
/// binding service client to specific service;
    ...
    z_control_effort_sub = nh.subscribe ( "right_wheel/controller_z",10,&DJIDrone::control_z_fromPID_callback,this );
#endif
//#else
    //Subscriber the apriltag_detection_pose
    apriltag_detection_subscriber = nh.subscribe ( "apriltag_detection/reldist",10,&DJIDrone::apriltag_datection_callback,this );
    // apriltag_detection_subscriber = nh.subscribe ( "apriltag_detection/reldist",10,&DJIDrone::apriltag_datection_callback,this );
...
//#endif
  ...
  //Send data to moblie device
  bool send_data_to_mobile_devide ( const std::vector<uint8_t> & data_,size_t  len )
  {
    // std::vector<uint8_t>  dd;
    //First declare a service;

    dji_sdk::SendDataToRemoteDevice send_data_to_mobile;
    //memcpy(send_data_to_mobile.request.data,data_,len);
    send_data_to_mobile.request.data =data_;
    return send_data_to_remote_device_service.call ( send_data_to_mobile ) &&send_data_to_mobile.response.result;
